...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "logs", label: "Nhật ký điều phối & gọi API" },
  { key: "image_tasks", label: "Lịch sử tác vụ ảnh" },
  { key: "accounts_snapshot", label: "Bản sao lưu tài khoản" },
  { key: "auth_keys_snapshot", label: "Bản sao lưu khóa người dùng" },
  { key: "images", label: "Thư mục hình ảnh" },
];
...
export function BackupSettingsCard() {
  ...
  const handleDownload = async (key: string, name: string) => {
    try {
      ...
      if (!authKey) {
        toast.error("Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại để tải về");
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          message = response.status === 401 ? "Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại" : message;
        }
      ...
      toast.success("Bắt đầu tải bản sao lưu");
    } catch (error) {
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold tracking-tight">Quản lý sao lưu (Cloudflare R2)</h2>
                <p className="text-sm text-stone-500">Tự động sao lưu dữ liệu quan trọng lên Cloudflare R2, hỗ trợ mã hóa, xoay vòng và khôi phục.</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={backupState?.running ? "warning" : backupState?.last_status === "success" ? "success" : "secondary"} className="rounded-md">
                {backupState?.running ? "Đang sao lưu" : backupState?.last_status === "success" ? "Thành công gần nhất" : backupState?.last_status === "error" ? "Thất bại gần nhất" : "Chưa thực hiện"}
              </Badge>
            </div>
          </div>

          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Tài khoản và khóa người dùng sẽ được trích xuất thành bản sao lưu logic. Thư mục ảnh mặc định không sao lưu để tránh tốn dung lượng.
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Tên Bucket R2</label>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Tiền tố sao lưu (Prefix)</label>
            ...
            <p className="text-xs text-stone-500">Tiền tố lưu trữ trong R2, ví dụ `backups/prod`.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Chu kỳ sao lưu định kỳ</label>
            ...
            <p className="text-xs text-stone-500">Tính bằng phút, hệ thống sẽ tự động sao lưu theo chu kỳ này.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Tự động dọn bản cũ khi tải bản mới lên. Nhập 0 để giữ lại tất cả.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Chỉ dùng khi bật mã hóa. Hãy bảo quản kỹ, nếu mất sẽ không giải mã được nội dung.</p>
          </div>
          </div>

          <div className="space-y-3 rounded-xl border border-stone-200 bg-white px-4 py-4">
          <div>
            <div className="text-sm font-medium text-stone-800">Dữ liệu sao lưu</div>
          ...
          <div>
            <div className="text-xs text-stone-500">Lần chạy gần nhất</div>
          ...
          <div>
            <div className="text-xs text-stone-500">File gần nhất</div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-stone-500">Xem thông tin file và quản lý bản sao lưu trên R2.</p>
            </div>
          </div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Chưa có bản sao lưu nào. Lưu cấu hình và nhấn "Sao lưu ngay" để tạo bản đầu tiên.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
                  ...
                  <div key={item.key} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                        ...
                        <span>Thời gian {formatDateTime(item.updated_at)}</span>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          <DialogHeader className="shrink-0 border-b border-stone-200 pb-3">
            <DialogTitle>Chi tiết bản sao lưu</DialogTitle>
          </DialogHeader>
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Không thể đọc chi tiết bản sao lưu. Nếu bản này có mã hóa, hãy nhập mật khẩu và lưu cấu hình trước.
              </div>
            ) : (
              <>
                <div className="grid gap-3 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4 text-sm text-stone-600 md:grid-cols-2">
                  ...
                  <div>
                    <div className="text-xs text-stone-500">Kiểu kích hoạt</div>
                  ...
                  <div className="md:col-span-2">
                    <div className="text-xs text-stone-500">Bộ lưu trữ</div>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Nội dung file</h4>
                  <div className="space-y-2">
                    {detail.files.map((item) => (
                      <div key={item.name} className="rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm">
                        ...
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                          <span>{item.exists ? "Đã bao gồm" : "Còn thiếu"}</span>
                          <span>Dung lượng {formatBytes(item.size)}</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Dữ liệu Snapshot</h4>
