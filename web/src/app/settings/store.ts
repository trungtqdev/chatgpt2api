...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  runBackup: async () => {
    ...
    try {
      ...
      toast.success(`Đã sao lưu thành công: ${data.result.key}`);
  ...
  testBackup: async () => {
    ...
    try {
      ...
      toast.success(`Kết nối R2 thành công (HTTP ${data.result.status})`);
    } catch (error) {
  ...
  resetRegister: async () => {
    ...
    try {
      ...
      set({ registerConfig: data.register });
      toast.success("Đã đặt lại số liệu thống kê");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Đặt lại thống kê thất bại");
    } finally {
      set({ isSavingRegister: false });
    }
  },
  ...
  browseFiles: async (pool) => {
    ...
    try {
      ...
      toast.success(`Đọc thành công ${files.length} tài khoản từ xa`);
    } catch (error) {
  ...
  startImport: async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Khởi động tác vụ nhập thất bại");
    } finally {
