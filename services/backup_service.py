...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("Môi trường hiện tại thiếu openssl, không thể thực hiện sao lưu mã hóa") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Sao lưu mã hóa thất bại: {detail or 'openssl thực thi thất bại'}") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("Môi trường hiện tại thiếu openssl, không thể giải mã nội dung sao lưu") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Giải mã sao lưu thất bại: {detail or 'openssl thực thi thất bại'}") from exc
    return result.stdout
...
class CloudflareR2Client:
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise BackupError(f"Cấu hình R2 không đầy đủ: thiếu {', '.join(missing)}")
    ...
    def test_connection(self) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Kết nối R2 thất bại: HTTP {response.status_code}")
    ...
    def upload_bytes(self, key: str, payload: bytes, *, content_type: str, metadata: dict[str, str] | None = None) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Tải lên bản sao lưu thất bại: HTTP {response.status_code}")
    ...
    def delete_object(self, key: str) -> None:
        ...
        if response.status_code >= 400 and response.status_code != 404:
            raise BackupError(f"Xóa bản sao lưu thất bại: HTTP {response.status_code}")

    def download_bytes(self, key: str) -> bytes:
        response = self._request("GET", key, timeout=60.0)
        if response.status_code >= 400:
            raise BackupError(f"Đọc bản sao lưu thất bại: HTTP {response.status_code}")
    ...
    def list_objects(self) -> list[dict[str, object]]:
        ...
        while True:
            ...
            if response.status_code >= 400:
                raise BackupError(f"Lấy danh sách bản sao lưu thất bại: HTTP {response.status_code}")
            text = response.text
...
class BackupService:
    ...
    def delete_backup(self, key: str) -> None:
        ...
        if not candidate:
            raise BackupError("Khóa đối tượng sao lưu không được để trống")
        client = CloudflareR2Client(config.get_backup_settings())
        try:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("Hiện chưa cấu hình mật khẩu mã hóa, không thể tải xuống và giải mã bản sao lưu đã mã hóa")
    ...
    def run_backup(self, *, trigger: str = "manual") -> dict[str, object]:
        with self._lock:
            ...
            if self._running:
                raise BackupError("Hiện đã có tác vụ sao lưu đang chạy")
            started_at = _iso_now()
            self._running = True
            save_backup_state({
    ...
    def _run_backup_once(self, *, trigger: str) -> dict[str, object]:
        ...
        if encrypted:
            ...
            if not passphrase:
                raise BackupError("Đã bật mã hóa sao lưu nhưng chưa đặt mật khẩu mã hóa")
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("Hiện chưa cấu hình mật khẩu mã hóa, không thể xem bản sao lưu đã mã hóa")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Phân tích gói nén sao lưu thất bại, bản sao lưu có thể đã bị hỏng") from exc
