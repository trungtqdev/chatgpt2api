...
DEFAULT_REVIEW_PROMPT = "Xác định xem yêu cầu của người dùng có được cho phép hay không. Chỉ trả lời ALLOW hoặc REJECT."
...
def check_request(text: str) -> None:
    ...
    for word in config.sensitive_words:
        if word in text:
            raise HTTPException(status_code=400, detail={"error": "Phát hiện từ khóa nhạy cảm, từ chối thực hiện yêu cầu này"})
    ...
    if result.startswith(("allow", "pass", "true", "yes", "通過", "通过", "允许", "安全", "hop le", "hợp lệ")):
        return
    raise HTTPException(status_code=400, detail={"error": "Kiểm duyệt AI không thông qua, từ chối thực hiện yêu cầu này"})
